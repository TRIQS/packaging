...
def packagePlatforms = ["noble", "resolute"]
for (int i = 0; i < packagePlatforms.size(); i++) {
  ...
  platforms["package-$platform"] = { -> node('docker') {
	...
	sh """#!/bin/bash -ex
	  top=\$PWD
	  for t in triqs triqs_dftkit triqs_cthyb triqs_ctseg triqs_dft_tools triqs_tprf triqs_maxent triqs_hubbardI triqs_hartree_fock solid_dmft triqs_Nevanlinna triqs_modest ; do
